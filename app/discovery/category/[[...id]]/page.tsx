...
export default async function CategoryDiscoveryPage({ params }: Props) {
  ...
  let category = null;
  let subCategories = allCategories.filter((c) => !c.parentId);
  let liquors: Awaited<ReturnType<typeof fetchLiquorsByCategories>> = [];

  if (categoryId) {
    [category] = await Promise.all([fetchCategory(categoryId)]);
    subCategories = allCategories.filter((c) => c.parentId === categoryId);
    const descendantIds = collectDescendantIds(categoryId, allCategories);
    liquors = await fetchLiquorsByCategories(descendantIds);
  }
