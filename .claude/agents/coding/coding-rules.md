...
テストランナーは **Vitest**（+ React Testing Library / jsdom）。詳細な規約・パターンチェックリスト・テスタブル設計は `docs/testing.md` を参照。

- テストはテスト対象と同じディレクトリに `<対象ファイル名>.test.ts(x)` として置く（コロケーション）
- `import { describe, it, expect } from 'vitest'` を明示する。グローバル注入は使わない
- 外部 API（aws-amplify 等）は必ずモックする — テストは AWS 認証情報・Sandbox なしで実行できなければならない
- 新しいロジック（判断・変換・バリデーション）を実装したら、純粋関数として切り出してテストを付ける
- 検証コマンド: `npm run test`（`npx tsc --noEmit`・`npm run lint` と並ぶ必須ゲート）
