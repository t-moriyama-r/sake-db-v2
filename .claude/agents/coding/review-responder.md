...
|---|---|---|
| `pr` | ✓ | 対象PR番号 |
| `task` | - | `fix`（修正のみ）/ `reply`（返信のみ）/ 省略時は両方実行 |

# 作業フロー

`task` の値によって実行するステップが変わる。

| task | 実行ステップ |
|---|---|
| 省略 | 1〜5 すべて実行 |
| `fix` | 1〜4（修正・プッシュのみ。返信はしない） |
| `reply` | 5 のみ（既に修正済みのコメントへ返信） |
...
## 2. ブランチに切り替え（fix / 省略 のみ）

```bash
gh pr checkout <pr>
```

## 3. coder に修正を委譲（fix / 省略 のみ）
